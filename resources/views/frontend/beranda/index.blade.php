        ...
        <div class="section__content">
            <div class="container">
                <div class="row">
                                ...
                                <div class="col-xl-3 col-lg-4 col-md-6 col-sm-6 u-s-m-b-30 filter__item">
                                    <div class="product-o product-o--hover-on product-o--radius">
                                        <div class="product-o__wrap">
                                            <a class="aspect aspect--bg-grey aspect--square u-d-block product-view" data-id="{{ $row->id }}" href="{{ route('shop.detail', $row->slug) }}">
                                                <img class="aspect__img" src="{{ asset('storage/uploads/products/' . $row->photo_name) }}" alt=""></a>
                                            <div class="product-o__action-wrap">
                                                <ul class="product-o__action-list">
                                                    <li>
                                                        <a href="javascript:(0);" id="addCart" class="product-view" data-id="{{ $row->id }}" title="Tambah Keranjang"><i class="fas fa-plus-circle"></i></a>
                                        ...
                                        <span class="product-o__category product-view" data-id="{{ $row->id }}">
                                            <a href="{{ route('shop.catalog', $row->catalog_slug) }}">{{ $row->catalog_name }}</a>
                                        </span>
                                        <span class="product-o__name product-view" data-id="{{ $row->id }}">
                                            <a href="{{ route('shop.detail', $row->slug) }}">{{ $row->name }}</a>
                                        </span>
                                        <div class="product-o__rating gl-rating-style">
                                            @for ($i = 1; $i <= 5; $i++) @if ($i <=$row->average_rating)
                                                <i class="fas fa-star"></i>
                                                @elseif ($i - $row->average_rating <= 0.5) <i class="fas fa-star-half-alt"></i>
                                                    @else
                                        ...
                                        <span class="product-o__price">
                                            ...
                                            <span class="product-o__discount">{{ $row->before_price ? 'Rp ' . number_format($row->before_price, 0, ',', '.') : '' }}</span></span>
                                    </div>
                                </div>
                                @empty
                    ...
                    <div class="col-lg-12">
                        <div class="load-more">
                            <button onclick="window.location.href='{{ route('shop.index') }}'" class="btn btn--e-brand" type="button">Memuat lebih banyak</button>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="owl-carousel product-slider" data-item="4">
                        ...
                        <div class="u-s-m-b-30">
                            <div class="product-o product-o--hover-on">
                                <div class="product-o__wrap">
                                    <a class="aspect aspect--bg-grey aspect--square u-d-block product-view" data-id="{{ $new_product->id }}" href="{{ route('shop.detail', $new_product->slug) }}">
                                        <img class="aspect__img" src="{{ asset('storage/uploads/products/' . $new_product->photos->first()->photo_name) }}" alt=""></a>
                                    <div class="product-o__action-wrap">
                                        <ul class="product-o__action-list">
                                            <li>
                                                <a href="javascript:(0);" id="addCart" data-id="{{ $new_product->id }}" title="Tambah Keranjang"><i class="fas fa-plus-circle"></i></a>
                                                <input type="hidden" name="qty" id="qty" value="1">
                                            </li>
                                        </ul>
                                    </div>
                                ...
                                <span class="product-o__category product-view" data-id="{{ $new_product->id }}">
                                    <a href="{{ route('shop.catalog', $new_product->catalog->slug) }}">{{ $new_product->catalog->name }}</a></span>
                                <span class="product-o__name product-view" data-id="{{ $new_product->id }}">
                                    <a href="{{ route('shop.detail', $new_product->slug) }}">{{ $new_product->name }}</a></span>
                                <div class="product-o__rating gl-rating-style">
                                    @for ($i = 1; $i <= 5; $i++) @if ($i <=$new_product->average_rating)
                                        <i class="fas fa-star"></i>
                                        @elseif ($i - $new_product->average_rating <= 0.5) <i class="fas fa-star-half-alt"></i>
                                            @else
                                            <i class="far fa-star"></i>
                                            @endif
                                            @endfor
                                ...
                                <span class="product-o__price">
                                    ...
                                    <span class="product-o__discount">{{ $new_product->before_price ? 'Rp ' . number_format($new_product->before_price, 0, ',', '.') : '' }}</span></span>
                            </div>
                        </div>
                        @empty
                        <h4 class="text-center">Data tidak tersedia</h4>
                    ...
                    <div class="owl-carousel" id="testimonial-slider">
                        ...
                        <div class="testimonial">
                            <div class="testimonial__img-wrap">
                                <img class="testimonial__img" src="{{ $row->user->avatar == '' ? 'https://ui-avatars.com/api/?background=random&name=' . $row->user->first_name . ' ' . $row->user->last_name : asset('storage/avatar/' . $row->user->avatar) }}" alt="">
                            </div>
                            <div class="testimonial__content-wrap">
                                ...
                                <span class="testimonial__author">{{ $row->user->first_name . ' ' . $row->user->last_name }}</span>
                            </div>
                        </div>
                        @endforeach
                    </div>
                ...
                <div class="slider-fouc">
                    <div class="owl-carousel" id="brand-slider" data-item="5">
                        ...
                        <div class="brand-slide">
                            <a href="{{ $row->slug }}">
                                <img src="{{ asset('storage/uploads/catalog/' . $row->image) }}" alt=""></a>
                        </div>
                        @endforeach
